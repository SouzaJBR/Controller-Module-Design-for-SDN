...
import time
import pox.lib.packet.igmpv3 as igmpv3
import random
...
class MulticastQoSController(EventMixin):
    ...
    def __init__(self):
        ...
        def startup():
            ...
            self.path_tree_map = defaultdict(
                lambda: None)  # self.path_tree_map[switch_dpid] = Complete path from receiver switch_dpid to src
            self.active_hosts = []
    ...
    def _handle_LinkEvent(self, event):
        ...
        if event.removed:
            ...
            log.info(
                'Removed Adjacency: Switch ' + str(switch1) + ' Port: ' + str(org_link.port1) + ' <----> Switch ' + str(
                    switch2) + ' Port: ' + str(org_link.port2))
                ...
                if alt_link.dpid1 == org_link.dpid1 and alt_link.dpid2 == org_link.dpid2:
                    if flip(alt_link) in core.openflow_discovery.adjacency:
                        ...
                        if alt_link.port1 in switch1.igmp_ports:
                            ...
                            log.warn(
                                str(alt_link.port1) + ' not found in ports of switch: ' + dpid_to_str(switch1.dpid))
                        ...
                        if alt_link.port2 in switch2.igmp_ports:
                            ...
                            log.warn(
                                str(alt_link.port2) + ' not found in ports of switch: ' + dpid_to_str(switch2.dpid))
            ...
            if self.adjacency[switch1][switch2] is None:
                ...
                if flip(org_link) in core.openflow_discovery.adjacency:
                    ...
                    log.info('Added Adjacency: Switch ' + str(switch1) + ' Port: ' + str(
                        org_link.port1) + ' <----> Switch ' + str(switch2) + ' Port: ' + str(org_link.port2))
    ...
    def _handle_PacketIn(self, event):
        ...
        if not igmp_pkt is None:
            ...
            log.debug(str(packetInSwitch) + ':' + str(event.port) + '| ' + str(igmp_pkt) + ' from Host: ' + str(
                ipv4_pkt.srcip))
            log.debug('')
            ...
            if self.flag_raise == 1:
                for switch_dpid in self.node_list:
                    ...
                    msg.openflow.command = of.OFPFC_DELETE
                ...
                if self.arestaGrupo.has_key(ipv4_pkt.dstip):
                    for aresta in self.arestaGrupo[ipv4_pkt.dstip]:
                        arestaGrafo = [i for i in self.weighted_topo_graph if i[0] == aresta[0] and i[1] == aresta[1]]
                        if arestaGrafo:
                            self.weighted_topo_graph.remove(arestaGrafo)
                            self.weighted_topo_graph.append((arestaGrafo[0], arestaGrafo[1], arestaGrafo[2] - 10))

                if self.arestaGrupo.has_key(ipv4_pkt.dstip):
                    log.debug(
                        'Arestas no grupo: ' + str(ipv4_pkt.dstip) + ' => ' + str(self.arestaGrupo[ipv4_pkt.dstip]))
                else:
                    log.debug('Arestas no grupo: ' + str(ipv4_pkt.dstip) + '=> Vazio')
        ...
        if not ipv4_pkt is None:
            ...
            if ipv4_pkt.dstip.inNetwork('224.0.0.0/4'):
                ...
                if not self.active_hosts is None:
                    ...
                    self.node_list = list(
                        self.node_set)  # correct  - Uncomment to observe that all nodes from graph are fetched by discovery.py
                    ...
                    for receiver in self.active_hosts:
                        ...
                        receiver_path = self.path_tree_map[
                            receiver[0]]  # getting tree for a particular receiver switch

                        log.debug('Path for ' + str(receiver[0]) + ' receiver is :' + str(
                            receiver_path))  # Prints path for clients
                    ...
                    self.arestaGrupo[ipv4_pkt.dstip] = list(edges_to_install)

                    for edge in edges_to_install:
                        arestaGrafo = [i for i in self.weighted_topo_graph if i[0] == edge[0] and i[1] == edge[1]]
                        if arestaGrafo:
                            self.weighted_topo_graph.remove(arestaGrafo)
                            self.weighted_topo_graph.append((arestaGrafo[0], arestaGrafo[1], arestaGrafo[2] + 10))
                        ...
                        if edge[0] in outgoing_flow_rules:
                            ...
                            output_port = self.adjacency[edge[0]][
                                edge[1]]  # adjacency = reouter , next reouter and port to next switch
...
def launch():
    core.registerNew(MulticastQoSController)
